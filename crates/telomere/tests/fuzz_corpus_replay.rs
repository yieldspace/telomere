...
const TARGET: &str = "parse_core_module";
...
#[test]
fn replays_parse_core_module_corpus() {
...
fn extract_quote_wat(
    ...
    match module.encode() {
        Ok(bytes) => insert_core_case(bytes, corpus, report),
...
fn insert_core_case(bytes: Vec<u8>, corpus: &mut BTreeMap<String, Vec<u8>>, report: &mut Report) {
    if !is_core_module(&bytes) {
        report.skip("non_core_header");
        return;
    }
...
fn is_core_module(bytes: &[u8]) -> bool {
    bytes.len() >= 8 && &bytes[..4] == WASM_MAGIC && bytes[4..8] == CORE_VERSION
}
