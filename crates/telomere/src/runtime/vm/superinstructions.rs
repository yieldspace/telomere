...
unsafe fn local_get4x3_add_const_binop_add(
    tail_code: *const Instr,
    ctx: &mut ExecuteContext,
) -> u32 {
    let first = unsafe { (*tail_code).operand.local_addr as usize };
    let second = unsafe { (*tail_code.add(1)).operand.local_addr as usize };
    let third = unsafe { (*tail_code.add(2)).operand.local_addr as usize };
    let kind = unsafe { (*tail_code.add(3)).operand.u32 };
    let rhs = unsafe { (*tail_code.add(4)).operand.i32 as u32 };
    let inner = local_u32_bits(ctx, second).wrapping_add(local_u32_bits(ctx, third));
    let adjusted = eval_stack_i32_const_binop(kind, inner, rhs).expect("invalid i32_const_binop");
    local_u32_bits(ctx, first).wrapping_add(adjusted)
}

/// WebAssembly `local.get x3; i32.add; i32.const; i32.binop; i32.add; local.set`.
///
/// This covers generic indexed-address arithmetic such as `base + ((x + y) << k)`.
///
/// # Safety
/// - `tail_code` must point to three local operands, const-binop kind/constant, destination,
///   and a skip width generated by the optimizer.
/// - The fused suffix must be the exact straight-line stack expression described above.
pub unsafe fn op_local_get4x3_i32_add_const_binop_i32_add_set4(
    tail_code: *const Instr,
    ctx: &mut ExecuteContext,
) -> VMResult<()> {
    dispatch_profile_count("op_local_get4x3_i32_add_const_binop_i32_add_set4");
    let result = unsafe { local_get4x3_add_const_binop_add(tail_code, ctx) };
    let dst = unsafe { (*tail_code.add(5)).operand.local_addr as usize };
    ctx.stack
        .local_set4_from_base_value(ctx.local_base_ptr, dst, result);
    let skip = unsafe { (*tail_code.add(6)).operand.u32 as isize };
    call_next(tail_code, skip, ctx)
}

/// WebAssembly `local.get x3; i32.add; i32.const; i32.binop; i32.add; local.tee`.
///
/// # Safety
/// - Same layout and validation contract as
///   `op_local_get4x3_i32_add_const_binop_i32_add_set4`.
pub unsafe fn op_local_get4x3_i32_add_const_binop_i32_add_tee4(
    tail_code: *const Instr,
    ctx: &mut ExecuteContext,
) -> VMResult<()> {
    dispatch_profile_count("op_local_get4x3_i32_add_const_binop_i32_add_tee4");
    let result = unsafe { local_get4x3_add_const_binop_add(tail_code, ctx) };
    let dst = unsafe { (*tail_code.add(5)).operand.local_addr as usize };
...
/// WebAssembly `local.get x3; i32.add; i32.const; i32.binop; i32.add; local.tee; i32.const; i32.store`.
///
/// This covers generic scaled-index address initialization such as
/// `*(base + ((x + y) << k)) = const` while preserving the `local.tee` side effect.
///
/// # Safety
/// - `tail_code` must point to three local operands, const-binop kind/constant, tee destination,
///   store constant, memory argument, and a skip width generated by the optimizer.
/// - The fused suffix must be the exact straight-line stack expression described above.
pub unsafe fn op_local_get4x3_i32_add_const_binop_i32_add_tee4_i32_const_store(
    tail_code: *const Instr,
    ctx: &mut ExecuteContext,
) -> VMResult<()> {
    dispatch_profile_count("op_local_get4x3_i32_add_const_binop_i32_add_tee4_i32_const_store");
    let result = unsafe { local_get4x3_add_const_binop_add(tail_code, ctx) };
    let dst = unsafe { (*tail_code.add(5)).operand.local_addr as usize };
    ctx.stack
        .local_set4_from_base_value(ctx.local_base_ptr, dst, result);
    let value = unsafe { (*tail_code.add(6)).operand.i32 as u32 };
    let memarg = unsafe { (*tail_code.add(7)).operand.memarg };
    let start = vm_try!(compute_memory_offset(memarg, result));
    vm_try!(unsafe { ctx.default_local_memory_mut_unchecked() }.write_u32_at(start, value));
    let skip = unsafe { (*tail_code.add(8)).operand.u32 as isize };
    call_next(tail_code, skip, ctx)
}
...
pub unsafe fn op_local_get4_run(tail_code: *const Instr, ctx: &mut ExecuteContext) -> VMResult<()> {
    ...
    debug_assert!((4..=16).contains(&count));
...
/// WebAssembly straight-line `local.get` run that skips consumed source handlers.
///
/// # Safety
/// - `tail_code` must point to a count operand, that many local operands, and a skip width.
/// - `ctx` must hold a valid frame and local base for the active function.
pub unsafe fn op_local_get4_run_skip(
    tail_code: *const Instr,
    ctx: &mut ExecuteContext,
) -> VMResult<()> {
    dispatch_profile_count("op_local_get4_run_skip");
    let count = (*tail_code).operand.u32 as usize;
    debug_assert!((4..=16).contains(&count));
    for index in 0..count {
        let local = (*tail_code.add(1 + index)).operand.local_addr as usize;
    ...
    let skip_slots = (*tail_code.add(1 + count)).operand.u32 as isize;
