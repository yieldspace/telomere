        ...
        match ctx.reader.read_exact_one()? {
            0x00 => {
                ...
                if export.is_none() {
                    ...
                    match sort.clone() {
                        SortWithIdx::Core(CoreSortWithIdx::Module(idx)) => AliasIdx::CoreModule(
                            ctx.validator.add_core_module(Binding::Alias(*idx))?,
                        ),
            ...
            0x02 => {
                ...
                match sort {
                    ...
                    Sort::Component => {
                        let component_idx =
                            target_validator.validate_component_idx(idx as usize)?;
                        AliasIdx::Component(
                            ctx.validator
