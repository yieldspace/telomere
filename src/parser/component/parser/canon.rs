...
pub fn parse_canon(
    ...
    let kind = match ctx.reader.read_exact_one()?.count(&mut counter) {
        0x00 => {
            ...
            let func_id = parse_func_id(ctx)?.count(&mut counter);
            let opts = parse_vec(ctx, |v| v.reader, parse_canon_opt)?.count(&mut counter);
            let ft = parse_type_id(ctx)?.count(&mut counter);
            CanonicalFuncKind::CanonLift(func_id, opts, ft)
        }
        0x01 => {
            assert_eq!(ctx.reader.read_exact_one()?.count(&mut counter), 0x00);
            let f = parse_func_id(ctx)?.count(&mut counter);
            let opts = parse_vec(ctx, |v| v.reader, parse_canon_opt)?.count(&mut counter);
            CanonicalFuncKind::CanonLower(f, opts)
        }
        0x02 => {
            let rt = parse_type_id(ctx)?.count(&mut counter);
        ...
        #[cfg(feature = "async")]
        0x0e => {
            let t = parse_type_id(ctx)?.count(&mut counter);
