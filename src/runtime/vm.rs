...
pub unsafe fn op_call(tail_code: *const Instr, ctx: &mut ExecuteContext) -> VMResult<()> {
    ...
    let funcaddr = ctx.instance().funcs.as_slice(&ctx.store.gc)[funcidx as usize];
...
unsafe fn internal_op_call_indirect(
    ...
    let table_addr = *vm_try!(VMResult::from_option(
        ctx.instance().tables.as_slice(&ctx.store.gc).get(tableidx),
...
pub unsafe fn op_global_get4(tail_code: *const Instr, ctx: &mut ExecuteContext) -> VMResult<()> {
    ...
    let addr = ctx.instance().globals.as_slice(&ctx.store.gc)[idx] as usize;
...
pub unsafe fn op_table_get(tail_code: *const Instr, ctx: &mut ExecuteContext) -> VMResult<()> {
    let idx = (*tail_code).operand.u32 as usize;
    let addr = ctx.instance().tables.as_slice(&ctx.store.gc)[idx] as usize;
...
pub unsafe fn op_table_init(tail_code: *const Instr, ctx: &mut ExecuteContext) -> VMResult<()> {
    ...
    let dst_table_addr = instance.tables.as_slice(&store.gc)[dst_table_idx] as usize;
        ...
        match &elem.init {
            ElemInit::FuncIdx(idxs) => {
                ...
                for (i, funcidx) in slice.iter().enumerate() {
                    dst[i] = instance.funcs.as_slice(&store.gc)[*funcidx as usize];
                }
            }
            ElemInit::ConstExpr(exprs) => {
                ...
                for (i, expr) in slice.iter().enumerate() {
                    dst[i] = vm_try!(execute_elem_init_const_expr(
                        global_store,
                        instance.globals.as_slice(&store.gc),
                        instance.funcs.as_slice(&store.gc),
                        expr,
                        dst_table.0.reftype,
                    ));
                }
...
pub unsafe fn op_table_copy(tail_code: *const Instr, ctx: &mut ExecuteContext) -> VMResult<()> {
    ...
    let src_table_addr = ctx.instance().tables.as_slice(&ctx.store.gc)[src_table_idx] as usize;
    let dst_table_addr = ctx.instance().tables.as_slice(&ctx.store.gc)[dst_table_idx] as usize;
...
pub unsafe fn op_table_grow(tail_code: *const Instr, ctx: &mut ExecuteContext) -> VMResult<()> {
    ...
    let table_addr = ctx.instance().tables.as_slice(&ctx.store.gc)[table_idx] as usize;
...
pub unsafe fn op_mem_size(tail_code: *const Instr, ctx: &mut ExecuteContext) -> VMResult<()> {
    if let Some(addr) = ctx.instance().mems.as_slice(&ctx.store.gc).get(0).copied() {
...
pub unsafe fn op_mem_init(tail_code: *const Instr, ctx: &mut ExecuteContext) -> VMResult<()> {
    ...
    let instance_id = ctx.instance_id();
    let memory = if let Some(v) = ctx.instance().mems.as_slice(&ctx.store.gc).get(0).copied() {
...
pub unsafe fn op_ref_func(tail_code: *const Instr, ctx: &mut ExecuteContext) -> VMResult<()> {
    ...
    vm_try!(ctx
        .stack
        .push_u32(ctx.instance().funcs.as_slice(&ctx.store.gc)[funcidx as usize]));
    call_next(tail_code, 1, ctx)
}
...
pub fn run_module_function(
    ...
    if let Some(ExportDesc::Func(idx)) = module_inst.exports.find(name) {
        let code_addr = funcs.as_slice(&store.gc)[idx.0 as usize];
...
pub fn get_global(instance: InstanceAddr, store: &mut Store, name: &str) -> VMResult<WasmValue> {
    ...
    if let Some(ExportDesc::Global(idx)) = module_inst.exports.find(name) {
        let addr = instance.globals.as_slice(&store.gc)[idx.0 as usize] as usize;
